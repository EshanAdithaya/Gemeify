...
const inputCls = 'w-full px-4 py-3.5 sm:py-3 text-sm rounded-sm border focus:outline-none focus:border-royal-400 transition-colors bg-slate-50 border-slate-200 text-slate-800 placeholder-slate-400';
...
function AboutContent() {
  ...
  return (
    <div className="min-h-screen bg-white pt-16 sm:pt-20">
      ...
      {/* ── Hero ── */}
      <section className="relative border-b border-slate-200 py-16 sm:py-20 px-4 sm:px-6 lg:px-8">
        <div className="absolute inset-0 bg-light-hero opacity-50" />
      ...
      {/* ── Stats ── */}
      <section className="border-b border-slate-200 py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
          {STATS.map(({ value, label }) => (
            <div key={label} className="text-center">
              ...
              <p className="text-[10px] sm:text-xs font-bold tracking-widest uppercase text-slate-500">{label}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="relative mt-4 lg:mt-0">
            ...
            <div className="absolute -bottom-4 -left-4 sm:-bottom-6 sm:-left-6 luxury-card p-4 sm:p-5">
              ...
              <p className="text-base sm:text-lg font-bold text-slate-800">25+ Years of Trust</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── Values ── */}
      <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 border-y border-slate-200 bg-slate-50">
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10 sm:mb-14">
            ...
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-900">Meet Our Gemologists</h2>
          </div>
            ...
            {TEAM.map((member) => (
              <div key={member.name} className="luxury-card overflow-hidden">
                ...
                <div className="p-5 sm:p-6">
                  <h3 className="text-lg font-bold text-slate-800 mb-0.5">{member.name}</h3>
                  <p className="text-royal-600 text-xs font-bold tracking-wider uppercase mb-1.5">{member.role}</p>
                  <p className="text-xs text-slate-500 mb-3">{member.credentials}</p>
      ...
      {/* ── Contact ── */}
      <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 border-t border-slate-200 bg-slate-50">
        <div className="max-w-5xl mx-auto">
          <div className="luxury-card p-6 sm:p-8 md:p-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 sm:gap-12">
              <div>
                <p className="section-label mb-4">Private Desk</p>
                <h2 className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight mb-4">Speak With an Expert</h2>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={text} className="flex items-center gap-3">
                      <Icon size={16} className="text-royal-600 flex-shrink-0" />
                      {href
                        ? <a href={href} className="text-sm text-slate-500 hover:text-royal-500 transition-colors">{text}</a>
              ...
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-4">
                {submitted ? (
                  <div className="flex flex-col items-center justify-center h-full gap-4 py-8 text-center">
                    <CheckCircle size={40} className="text-royal-600" />
                    <p className="text-xl font-bold text-slate-900">Message Received</p>
                    <p className="text-sm text-slate-500">Our team will respond within 24 hours.</p>
                  </div>
                ) : (
                  <>
                    {[['name','Name','text'], ['email','Email Address','email']].map(([id, label, type]) => (
                      <div key={id}>
                        <label className="block text-[10px] font-bold tracking-widest uppercase mb-2 text-slate-500">{label}</label>
                    ...
                    <div>
                      <label className="block text-[10px] font-bold tracking-widest uppercase mb-2 text-slate-500">Message</label>
